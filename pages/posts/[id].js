...
import postsPageInitialPathsCede2Resource from '../../resources/posts-page-initial-paths-cede2'
import postsPageInitialProps803daResource from '../../resources/posts-page-initial-props-803da'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Lead Assurance Architect</title>
          <meta
            property="og:title"
            content="Posts1 - Lead Assurance Architect"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsCede2Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps803daResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
